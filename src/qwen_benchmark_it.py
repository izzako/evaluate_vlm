...
class VLMBenchmark:
    ...
    def tile_and_stack_vertically(self, image, tile_width=1024):
        """Split wide image into tiles and stack them vertically"""
        width, height = image.size
        tiles = []
        
        # Split into tiles
        for x in range(0, width, tile_width):
            box = (x, 0, min(x + tile_width, width), height)
            tile = image.crop(box)
            tiles.append(tile)
        ...
        new_height = height * num_tiles
        
        # Create new image with stacked tiles
        stacked = Image.new(image.mode, (new_width, new_height))
        
        # Paste each tile vertically
        for i, tile in enumerate(tiles):
            y_position = i * height
            stacked.paste(tile, (0, y_position))
    ...
    async def inference_single(
        ...
        try:
            ...
            image_url = self.get_pil_image_url(self.tile_and_stack_vertically(pil_image))
        ...
        try:
            ...
            if fewshot_examples:
                for example in fewshot_examples:
                    ...
                    example_image_url = self.get_pil_image_url(self.tile_and_stack_vertically(example_pil))
                    messages.append({
                        "role": "user",
                        "content": [
